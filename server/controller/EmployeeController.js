...
export const getEmployee=async(req,res)=>{
  ...
  return res.json(result)
    ...
    console.log(error.message,"Error occured in getting employees")
...
export const updateEmployee=async(req,res)=>{
  ...
  const userFields = { email, role: role || "EMPLOYEE" };
    if (password) userFields.password = await bcrypt.hash(password, 10);
    await User.findByIdAndUpdate(emp.userId, userFields); 
    const updatesEmp=await Employee.findByIdAndUpdate(id,{firstName,lastName,email,phone,position,department,basicSalary:Number(basicSalary)||0,deductions:Number(deductions)||0,allowances:Number(allowances)||0,joinDate:new Date(joinDate),bio:bio||""})
